...
# с помощью регулярного выражения тебе нужно достать по каждому юзеру инфу из документа и закинуть в обьект Client
...
# Хеш таблица будет содержать ключи которые будут равны имени колонки из CSV файла а значение то что вернула регулярка
...
# Далее нужно описать класс Client в котором конструктор принимает параметры с именами из имен колонок и записывает их в свои поля экземпляра. 
